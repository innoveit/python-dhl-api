...
from python_dhl.resources.helper import ProductCode, ShipmentType, TypeCode, next_business_day, IncotermCode, \
    MeasurementUnit, ShipperType, DocumentType, AccountType
...
class TestDhl(unittest.TestCase):
    ...
    def test_shipment(self):
        ...
        rates = service.get_rates(sender_address, receiver_address, packages[0], shipment_date)
        print('\n+++++++++++++++++++++++')
    ...
    def test_shipment_with_customs_payed_by_shipper(self):
        ...
        accounts = [
            shipment.DHLAccountType(type_code=AccountType.SHIPPER, number=Setting.DHL_ACCOUNT_EXPORT),
            shipment.DHLAccountType(type_code=AccountType.DUTIES_TAXES, number=Setting.DHL_ACCOUNT_EXPORT),  # here it specifies that the shipper is also paying the duties
        ]
    ...
    def test_upload_document(self):
        ...
        document_image = [shipment.DHLDocumentImage(
            type_code=DocumentType.INV.name,
            image_format='pdf',
            content=LOGO_BASE64
        )]
